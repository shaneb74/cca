    ...
    if overlay_path and Path(overlay_path).exists():
        ...
        for gid, ov in overrides.items():
            ...
            for f in g.get("fields", []):
                # apply wildcard defaults then specific overrides
                for k, v in wildcard.items():
                    f.setdefault(k, v)
                this_ov = field_ovs.get(f.get("label", f.get("field", "")), {})
...
def apply_ui_group_answers(groups_cfg, grouped_answers, existing_fields=None):
    ...
    for gid, answers in grouped_answers.items():
        cfg = groups.get(gid)
        if not cfg:
            # fall back to direct passthrough (e.g., spouse income fallback)
...
def compute(spec, inputs):
    ...
    def per_person_cost(person_key):
        ...
        if care_type == "In-Home Care (professional staff such as nurses, CNAs, or aides)":
            ...
            # interpolate hourly from matrix keys
...
HELP_HOURS = "Choose average paid caregiver hours per day (0–24). This is professional in‑home care time."
...
# ---------- Step 2: Care plans ----------
    ...
    def render_person_care(person_key, name_label, with_paid_toggle=False):
        ...
        if care_type.startswith("In-Home Care"):
            ...
            st.selectbox(
                "Room type",
                list(spec["lookups"]["room_type"].keys()),
                index=0,
                key=f"room_{person_key}",
                help="Studio = lowest cost; 1BR = standard; 2BR = largest. Actual costs vary by facility."
            )
...
# ---------- Step 3: Financials ----------
    ...
    def render_benefits_group(person_label: str, gid: str):
        ...
        with st.expander(f"{g['label'].replace('Person A', person_label).replace('Person B', person_label)} — {g.get('prompt','')}", expanded=False):
            ans = {}
            # VA selector is always shown; benefit applies only if eligible is checked
    ...
    def render_group(gid, rename_map=None):
        ...
        with st.expander(f"{heading} — {g.get('prompt','')}", expanded=False):
            ...
            for f in g["fields"]:
                label = f.get("label", f["field"])
                # Backward‑compatible rename if an older JSON is used
                ...
                if gid == "group_assets":
                    if "Other savings & investments" in label: help_txt = "Checking/savings, brokerage, CDs, spendable 401k/IRA, cash reserves."
                # render
                if kind == "boolean":
        ...
        for cat, gids in CATEGORY_MAP.items():
            ...
            for gid in gids:
                ...
                if gid.startswith("group_benefits"):
                    ...
                    # If spouse is covered by A's VA tier or combined tier, suppress B's VA
                    if gid.endswith("_person_b") and (st.session_state.va_spouse_is_b or st.session_state.va_both_vets_combined):
                        ...
                        # spouse logic hooks based on A's selection
                        if gid.endswith("_person_a"):
                            va_choice = st.session_state.get(f"va_tier_{gid}")
                            va_elig = st.session_state.get(f"va_elig_{gid}", False)
                            if va_choice == "Veteran with spouse" and st.session_state.get("include_b"):
                                is_spouse_b = st.checkbox(f"Is {name_map.get('B','Person B')} the spouse included in this plan?", value=True, key="va_is_spouse_b")
                                st.session_state.va_spouse_is_b = bool(va_elig and is_spouse_b)
                                st.session_state.va_both_vets_combined = False
                            elif va_choice == "Two married veterans (both A&A)" and st.session_state.get("include_b"):
                                ...
                                st.session_state.va_both_vets_combined = bool(va_elig)
                            else:
                                st.session_state.va_spouse_is_b = False
                                if va_choice != "Two married veterans (both A&A)":
                                    st.session_state.va_both_vets_combined = False
                    if ans is not None:
