...
# streamlit_app.py
# Senior Care Cost Planner — full app with VA rules, home mods, modern assets UI.
# Reads optional overrides from "senior_care_modular_overlay.json".
...
DEFAULT_VA_CATEGORIES = {
    ...
    "Surviving spouse + 1 child (A&A)": 1808.00
}

DEFAULT_HOME_MOD_SPECS = {
...
def load_spec(base_path: str, overlay_path: Optional[str] = None) -> Dict[str, Any]:
    ...
    if not isinstance(lookups.get("va_categories"), dict) or not lookups.get("va_categories"):
        lookups["va_categories"] = dict(DEFAULT_VA_CATEGORIES)
    if not isinstance(lookups.get("home_mod_specs"), dict):
        lookups["home_mod_specs"] = dict(DEFAULT_HOME_MOD_SPECS)
    return spec
...
def compute_results(inputs: Dict[str, Any], spec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Detailed assets roll-up (granular + totals; backwards-compatible)
    detailed_asset_keys = [
        "ira_traditional", "ira_roth", "ira_sep", "ira_simple",
        "employer_401k", "employer_403b", "employer_457b",
        "brokerage_taxable", "cash_savings", "cds_balance",
        "hsa_balance", "annuity_surrender", "life_cash_value",
        "other_assets", "other_assets_grouped", "home_equity"
    ]
    aggregate_asset_keys = ["ira_total", "employer_retirement_total"]
    total_assets = 0.0
    for k in aggregate_asset_keys:
        total_assets += float(inputs.get(k, 0.0))
    for k in detailed_asset_keys:
        total_assets += float(inputs.get(k, 0.0))
    ...
    # Step 3
        ...
        def va_block(person_key: str, person_label: str):
            ...
            if override:
                ...
                preview = compute_results(inp, spec)
                auto_val = preview.get("va_a", 0.0) if person_key == "person_a" else preview.get("va_b", 0.0)
                st.text_input(
        ...
        def render_group_live(gid: str):
            ...
            if not g:
                return
            # NEW: allow overlay to hide groups entirely
            if g.get("module") == "hidden":
                return
            ...
            expanded = gid.startswith("group_income") or gid in {"group_assets_common"}
...
- The **VA benefit (auto)** field below shows the computed monthly amount and updates as you edit income or medical items.
        ...
        with st.expander("Home modifications (one-time costs)", expanded=False):
            ...
            specs = spec.get("lookups", {}).get("home_mod_specs", DEFAULT_HOME_MOD_SPECS)
            ...
            for key, cfg in specs.items():
                ...
                with cols[0]:
                    st.caption(cfg.get("note",""))
                default_pick = st.session_state.get(f"hm_pick_{key}", "Typical")
                with cols[1]:
                    pick = st.radio("Pick a level", ["Basic","Typical","Extensive","Custom"],
                                    index={"Basic":0,"Typical":1,"Extensive":2,"Custom":3}.get(default_pick,1),
                                    horizontal=True, label_visibility="collapsed", key=f"hm_pick_{key}")
                if pick == "Basic":
                    value = float(cfg["min"])
                elif pick == "Typical":
                    value = float(cfg["avg"])
                    ...
                    default = _clamp(inp.get(f"hm_{key}", cfg["avg"]), cfg["min"], cfg["max"])
                    value = st.number_input("Custom amount", min_value=float(cfg["min"]), max_value=float(cfg["max"]),
                                            value=float(default), step=float(cfg["step"]), format="%.2f", key=f"hm_input_{key}")
                with cols[2]:
